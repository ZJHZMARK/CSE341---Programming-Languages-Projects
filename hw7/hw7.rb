...
class Point < GeometryValue
  ...
  def intersectPoint(other)
    if real_close_point(other.x, other.y, @x, @y)
      Point.new(@x, @y)
    else
      NoPoints.new
    end
  ...
  def intersectWithSegmentAsLineResult(seg)
    if inbetween(@x, seg.x2, seg.x1) && inbetween(@y, seg.y2, seg.y1)
      Point.new(@x, @y)
    else NoPoints.new
    end
  end
...
class LineSegment < GeometryValue
  ...
  def intersectWithSegmentAsLineResult(seg)
    ...
    if real_close(@x2, @x1)
      (if @y2 < seg.y2
       (aXend,aYend,aXstart,aYstart) = @x1, @y1, @x2, @y2
       (bXend,bYend,bXstart,bYstart) = seg.x1, seg.y1, seg.x2, seg.y2
      else
        (aXend,aYend,aXstart,aYstart) =  seg.x1, seg.y1, seg.x2, seg.y2
        (bXend,bYend,bXstart,bYstart) =  @x1, @y1, @x2, @y2
       end)
